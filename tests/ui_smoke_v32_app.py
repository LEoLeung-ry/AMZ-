...
def synthetic_raw(market_code: str) -> pd.DataFrame:
    ...
    return pd.DataFrame(
        {
            "分类ID": [f"100{index + 1}" for index in range(9)],
            "中文名称": ["日伞", "充电宝", "垃圾袋", "收纳箱", "大豆蛋白粉", "啤酒", "塔扇", "电动剃须刀", "系统"],
            "分类名称": [
                "Parasol",
                "Power Bank",
                "Trash Bags",
            ...
            "根类目": ["fashion", "electronics", "home", "home", "food-beverage", "food-beverage", "home", "beauty", "home"],
            "Node Path": [
            ...
            "近12个月销量": [80000, 120000, 130000, 90000, 90000, 100000, 140000, 110000, 150000],
            "近12个月净销售额": [1800000000, 3500000000, 3200000000, 2100000000, 4000000000, 5000000000, 3800000000, 2900000000, 4500000000],
            "近12个月搜索量": [1300000, 2500000, 2800000, 1700000, 1800000, 2000000, 3000000, 2200000, 3200000],
            "近12个月点击量": [300000, 600000, 650000, 350000, 450000, 500000, 700000, 520000, 720000],
            "近12个月浏览量": [700000, 1200000, 1300000, 800000, 900000, 1000000, 1400000, 1050000, 1450000],
            "平均价格": [22500, 29167, 24615, 23333, 44444, 50000, 27143, 26364, 30000],
            "ASIN数量": [100, 300, 220, 180, 180, 200, 260, 210, 400],
            "最受欢迎关键词": ["parasol", "power bank", "trash bags", "storage boxes", "soy protein", "beer", "tower fan", "shaver", "systems"],
            "最受欢迎关键词值": [70000, 120000, 140000, 80000, 90000, 100000, 150000, 110000, 160000],
            "价格转化率最大值": [f"{symbol}20 - {symbol}40"] * 9,
            "4星及以上评分数量": [70, 220, 180, 140, 140, 150, 200, 170, 250],
            "3星评分数量": [10, 30, 25, 20, 20, 20, 28, 22, 30],
            "2星评分数量": [8, 20, 15, 10, 10, 10, 18, 12, 20],
            "1星评分数量": [5, 10, 8, 5, 5, 5, 9, 6, 10],
        }
    )
...
scored = add_actionability_v32(add_strategy_robustness(all_data, decision))
market_code = "JP"
filtered = scored.loc[
    scored["MarketCode"].eq(market_code)
    & scored["DefaultBusinessEligible"].fillna(False)
    & ~scored["BroadNodeWarning"].fillna(False)
].copy()
