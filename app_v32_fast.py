...
APP_VERSION = "2026.07.29-v3.2.2-beta"
PAGE_OPTIONS = [
    "🌐 全球总览",
    "🏆 候选排行榜",
    "🧪 策略稳健性",
...
@st.cache_data(ttl=1800, show_spinner="正在读取并标准化四站类目数据…")
def cached_load_all():
...
@st.cache_data(ttl=1800, show_spinner="正在计算机会分、规则预警和五策略稳健性…")
...
with st.sidebar:
    st.markdown("## 🌍 全球类目情报")
    st.caption(f"{APP_VERSION} · 按页面延迟渲染")
    page = st.radio("功能页面", PAGE_OPTIONS, index=0, key="fast_page")
    st.divider()
...
with st.sidebar:
    ...
    selected_entry_classes = st.multiselect(
        ...
        key="fast_entry_classes",
    )
    only_physical = st.toggle("排除数字商品高风险节点", True, key="fast_only_physical")
    exclude_broad_nodes = st.toggle(
        "排除名称过宽节点",
        True,
        help="隐藏Systems、Strips、Powders等难以直接定义产品的聚合名称。",
        key="fast_exclude_broad",
    )
    min_confidence = st.slider("最低数据置信度", 0, 100, 50, 5, key="fast_min_confidence")
    min_evidence = st.slider("最低证据字段覆盖", 0, 100, 0, 5, key="fast_min_evidence")
    min_rank_score = st.slider("最低排序分", 0, 100, 0, 1, key="fast_min_rank")
    ...
    st.divider()
    if st.button("↻ 强制刷新四站数据", use_container_width=True, key="fast_refresh"):
...
st.markdown(
    ...
    <section class="hero">
      ...
      <p>默认不使用历史品类偏好。仅渲染当前页面，减少无效计算和页面等待。</p>
      <div class="meta-row">
